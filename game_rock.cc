...
{
    int result;
    std::cout <<"Возьмите от 1 до 3 камушки" << std::endl;
    std::string input;
    ...
    while ( f || result < 1 || result > 3) {//обработка некоректного ввода используя строку
        std::cerr << "Неверный ввод, можно брать только от 1 до 3 камней. Введите количество камней еще раз: " << std::endl;
    	std::getline(std::cin, input);
    	std::stringstream input_ss { input };
        input_ss >> result;
        f = input_ss.fail();
...
int lvl_1(bool f)
{
	...
	if(f1) std::cout << "Игра закончилась. Победил компьютер" << std::endl;
	else std::cout << "Игра закончилась. Вы победили" << std::endl;
	return 1;
}
...
void lvl_3(bool f, int ans)
{
	int d1 = ans;
	std::cout << "Всего " << d1 << " камней" << std::endl;
	int d2, a;//Для рандомного хода компьютера
	bool f1;
	for (unsigned i=0; d1>0; ++i)
	{
		if(i%2 == 0)
	...
	if(f1) std::cout << "Игра окоооооонечна. Победил компьютер =(" << std::endl;
...
int main()
{
	...
	do
	{
		std::cout << "Выберите уровень сложности : " << std::endl
				  << "1. Легкий уровень" << std::endl
				  << "2. Средний уровень" << std::endl
			      << "3. Высокий уровень" << std::endl
				  << "4. Выход из игры" << std::endl;
		std::cin >> choise;
